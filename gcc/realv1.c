		    ...
		    const REAL_VALUE_TYPE *r)
{
  ...
  fprintf(stderr,"r->sig0 = %ld\n",r->sig[0]);
  fprintf(stderr,"r->sig0 = %ld\n",(r->sig[0] >> (HOST_BITS_PER_LONG - 24)));
  printBits(sizeof((r->sig[SIGSZ-1] )),&(r->sig[SIGSZ-1]));
  sig = (r->sig[SIGSZ-1] >> (HOST_BITS_PER_LONG - 24));
  printBits(sizeof((r->sig[SIGSZ-1] >> (HOST_BITS_PER_LONG - 24))),&sig);
  //fprintf(stderr,"r->sig1 = %ld\n",(r->sig[1] >> (HOST_BITS_PER_LONG - 24)));
  ...
  if (Wposit){
      //fprintf(stderr,"Befor img = %")
      printBits(sizeof(image), &image);
      printBits(sizeof(*(float*)&image), &(*(float*)&image));
