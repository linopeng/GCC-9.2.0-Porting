            ...
            const REAL_VALUE_TYPE *r)
{
  ...
  int fracLength,exp;
  //SIGNIFICAND_BITS = (128 + HOST_BITS_PER_LONG)
  //SIGSZ = (SIGNIFICAND_BITS / HOST_BITS_PER_LONG)
  //SIG_MSB = ((unsigned long)1 << (HOST_BITS_PER_LONG - 1))
  //real_format's significand is 0.[real_sig] and IEEE is 1.[ieee_sig]
  //which means if real_sig start from 0 ,it must be denormalized
  bool denormal = (r->sig[SIGSZ-1] & SIG_MSB) == 0;
fprintf(stderr,"This is test\n");
  image = sign << 31;
  //posit32 with es=2 have at most 28 significand bits
  //for the hidden 1 ,use mask to cut off the first bit 
