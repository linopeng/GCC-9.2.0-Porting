...
static bool spim_hard_regno_mode_ok_p[MAX_MACHINE_MODE][FIRST_PSEUDO_REGISTER];
static bool
spim_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
	return spim_hard_regno_mode_ok_p[mode][regno];
}
...
spim_modes_tieable_p (machine_mode mode1, machine_mode mode2)
{
  ...
  return spim_modes_tieable_p (mode1 , mode2);
}

#define REG_CLASS_FROM_LETTER(ch) NO_REGS 

#define STARTING_FRAME_OFFSET spim_starting_frame_offset ()
...
int
spim_legitimate_constant_p (rtx X)
{
