...
{
	if (is_caller_saved_reg(REGN) || is_callee_saved_reg(REGN) || is_arg_reg(REGN) || is_return_val_reg(REGN) || (REGN >= 28 && REGN <= 31) || (REGN == 1))
		return IITB_YES;
	return IITB_NO;
}
...
int modes_tieable_p(enum machine_mode MODE1, enum machine_mode MODE2)
{
	if ((MODE1 == MODE2) || (GET_MODE_SIZE(MODE1) <= GET_MODE_SIZE(MODE2) && GET_MODE_CLASS(MODE1) == GET_MODE_CLASS(MODE2)))
		return IITB_YES;
	return IITB_NO;
}
...
{
	if (is_base_reg(REGN) || (REGN >= FIRST_PSEUDO_REGISTER && is_base_reg(reg_renumber[REGN])))
		return IITB_YES;
	return IITB_NO;
}
...
{
	if (is_index_reg(REGN) || (REGN >= FIRST_PSEUDO_REGISTER && is_index_reg(reg_renumber[REGN])))
		return IITB_YES;
	return IITB_NO;
}
...
int constant_address_p(rtx X)
{
	...
	return (CONSTANT_P(X) &&
			GET_CODE(X) != CONST_DOUBLE && GET_CODE(X) != CONST_VECTOR);
}
...
{
	//fprintf(stderr, "--------This is legitimate_address2 line 228--------\n");
	rtx op1, op2;
	if (CONSTANT_ADDRESS_P(X))
		return 1;
...
{
	//fprintf(stderr, "--------This is print_operand_address line 394 --------\n");
	rtx op1, op2, temp;
	int num;
	switch (GET_CODE(X))
	{
	...
	case PLUS:
		...
		op1 = XEXP(X, 0);
		op2 = XEXP(X, 1);
		if (GET_CODE(op1) == CONST_INT && (GET_CODE(op2) == REG || GET_CODE(op2) == SUBREG))
		/*base displacement*/
		{
...
int asm_output_symbol_ref(FILE *stream, rtx sym)
{
	fprintf(stderr, "-----------------line 472-------------\n");
	//fprintf(stream,"_");
...
void spim_init_cumulative_args(CUMULATIVE_ARGS *cum, tree fntype)
{
...
char *
conditional_insn(enum rtx_code code, rtx operands[], int isRev)

{
	if (!isRev) /*Normal condition*/
	{
		fprintf(stderr, "spim.c line 574\n");
		switch (code)
		{
		case EQ:
			return "beq \t%1, %2, %l3";
	...
	{
		fprintf(stderr, "spim.c line 600\n");
		switch (code)
		{
		case EQ:
			return "bne \t%1, %2, %l3";
...
void spim_asm_internal_label(FILE *stream, const char *prefix, unsigned int labelno)
{
	fprintf(stream, "%s%d:\n", prefix, labelno);
	return;
}
...
int spim_starting_frame_offset(void)
{
	return 0;
}
				  ...
				  machine_mode mode, const_tree type, bool named)
{
	...
	doubleword_aligned_p = (spim_function_arg_boundary(mode, type) > BITS_PER_WORD);
							  ...
							  addr_space_t as ATTRIBUTE_UNUSED)
{
	...
	if (GET_CODE(addr) == PLUS && symbolic_operand(XEXP(addr, 1), VOIDmode))
		return true;
