                   ...
                   unsigned int n)
{
  ...
  if (n == 0)
  {
    ...
    for (i = 0; i < SIGSZ; ++i)
    {
      r->sig[SIGSZ - 1 - i] = (((ofs + i >= SIGSZ ? 0 : a->sig[SIGSZ - 1 - i - ofs]) << n) | ((ofs + i + 1 >= SIGSZ ? 0 : a->sig[SIGSZ - 1 - i - ofs - 1]) >> (HOST_BITS_PER_LONG - n)));
    }
}
            ...
            const REAL_VALUE_TYPE *b)
{
  ...
  for (i = 0; i < SIGSZ * 2; ++i)
  {
    ...
    for (j = 0; j < 2; ++j)
    {
      int exp = (REAL_EXP(a) - (2 * SIGSZ - 1 - i) * (HOST_BITS_PER_LONG / 2) + (REAL_EXP(b) - (1 - j) * (HOST_BITS_PER_LONG / 2)));
...
bool real_arithmetic(REAL_VALUE_TYPE *r, int icode, const REAL_VALUE_TYPE *op0,
...
bool real_compare(int icode, const REAL_VALUE_TYPE *op0,
                  const REAL_VALUE_TYPE *op1)
{
  enum tree_code code = (enum tree_code)icode;
...
bool real_can_shorten_arithmetic(machine_mode imode, machine_mode tmode)
{
  ...
  /* These conditions are conservative rather than trying to catch the
     ...
     and double.  */
  return (ifmt->b == tfmt->b && ifmt->p > 2 * tfmt->p && ifmt->emin < 2 * tfmt->emin - tfmt->p - 2 && ifmt->emin < tfmt->emin - tfmt->emax - tfmt->p - 2 && ifmt->emax > 2 * tfmt->emax + 2 && ifmt->emax > tfmt->emax - tfmt->emin + tfmt->p + 2 && ifmt->round_towards_zero == tfmt->round_towards_zero && (ifmt->has_sign_dependent_rounding == tfmt->has_sign_dependent_rounding) && ifmt->has_nans >= tfmt->has_nans && ifmt->has_inf >= tfmt->has_inf && ifmt->has_signed_zero >= tfmt->has_signed_zero && !MODE_COMPOSITE_P(tmode) && !MODE_COMPOSITE_P(imode));
}
...
void real_to_decimal_for_mode(char *str, const REAL_VALUE_TYPE *r_orig,
                              ...
                              int crop_trailing_zeros, machine_mode mode)
{
  ...
  if (cmp_one > 0)
  {
    ...
    SET_REAL_EXP(&u, REAL_EXP(&u) + REAL_EXP(&r) - (SIGNIFICAND_BITS - 1));
    r = u;
...
void real_to_decimal(char *str, const REAL_VALUE_TYPE *r_orig, size_t buf_size,
...
void real_to_hexadecimal(char *str, const REAL_VALUE_TYPE *r, size_t buf_size,
                         size_t digits, int crop_trailing_zeros)
{
...
int real_from_string(REAL_VALUE_TYPE *r, const char *str)
{
  ...
  if (str[0] == '0' && (str[1] == 'x' || str[1] == 'X'))
  {
    ...
    while (1)
    {
      ...
      if (pos >= 0)
      {
        r->sig[pos / HOST_BITS_PER_LONG] |= (unsigned long)d << (pos % HOST_BITS_PER_LONG);
        pos -= 4;
      }
      else if (d)
...
void real_from_integer(REAL_VALUE_TYPE *r, format_helper fmt,
...
bool real_nan(REAL_VALUE_TYPE *r, const char *str, int quiet,
              format_helper fmt)
{
  if (*str == 0)
  {
...
void real_convert(REAL_VALUE_TYPE *r, format_helper fmt,
...
long real_to_target(long *buf, const REAL_VALUE_TYPE *r_orig,
                    format_helper fmt)
{
  REAL_VALUE_TYPE r;
  long buf1;
...
decode_ieee_single(const struct real_format *fmt, REAL_VALUE_TYPE *r,
                   const long *buf)
{
  fprintf(stderr, "decode decode decode ieee\n");
  ...
  else if (exp == 255 && (fmt->has_nans || fmt->has_inf))
  {
    if (image)
    {
      r->cl = rvc_nan;
      r->sign = sign;
      r->signalling = (((image >> (HOST_BITS_PER_LONG - 2)) & 1) ^ fmt->qnan_msb_set);
      r->sig[SIGSZ - 1] = image;
    }
    else
    {
...
bool real_powi(REAL_VALUE_TYPE *r, format_helper fmt,
...
void real_trunc(REAL_VALUE_TYPE *r, format_helper fmt,
...
void real_floor(REAL_VALUE_TYPE *r, format_helper fmt,
                const REAL_VALUE_TYPE *x)
{
  REAL_VALUE_TYPE t;
...
void real_ceil(REAL_VALUE_TYPE *r, format_helper fmt,
               const REAL_VALUE_TYPE *x)
{
  REAL_VALUE_TYPE t;
...
void real_round(REAL_VALUE_TYPE *r, format_helper fmt,
                const REAL_VALUE_TYPE *x)
{
...
bool real_nextafter(REAL_VALUE_TYPE *r, format_helper fmt,
...
void get_max_float(const struct real_format *fmt, char *buf, size_t len)
{
  int i, n;
  char *p;
